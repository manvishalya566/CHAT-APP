  ...
  return selectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        <img src={selectedUser.profilePic || assets.avatar_icon} alt={selectedUser.fullName} className="w-8 h-8 rounded-full" />

        <p className="flex-1 text-lg text-white flex items-center gap-2">
          {selectedUser.fullName}
          {onlineUsers.includes(selectedUser._id)}<span className={`w-2 h-2 rounded-full ${selectedUser.isOnline ? 'bg-green-500' : 'bg-gray-500'}`} />
        </p>

        <img onClick={() => setSelectedUser(null)} src={assets.arrow_icon} alt="Back" className="md:hidden w-7 cursor-pointer" />
        <img src={assets.help_icon} alt="" className="max-md:hidden w-5 cursor-pointer" />
      </div>
      {/*chat area*/}
    ...
    <div className="flex flex-col items-center justify-center h-full text-center">
      <img onClick={handleSendMessage}src={assets.logo_icon} className="w-16 mb-3" alt="QuickChat logo" />
